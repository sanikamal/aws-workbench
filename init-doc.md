...
> Reusable tools, automation, templates, and deployment utilities for AWS, data engineering, machine learning, and AI.
...
**CloudForge** is my personal engineering toolkit for turning repetitive real-world development tasks into reusable tools, automation, templates, and engineering patterns.

It is built from practical work across **AWS, Data Engineering, Machine Learning, and AI**.

Instead of repeatedly solving the same infrastructure, deployment, packaging, data, or AI engineering problem from scratch, CloudForge provides a place to:

* Build a solution once.
* Make it reusable.
* Improve it over time.
* Apply it across projects, accounts, regions, and environments.
* Add new capabilities whenever a recurring problem appears.

CloudForge is intended to grow organically from real engineering work.
...
New Engineering Task
     ↓
Does CloudForge already have a solution?
     ↓
 ┌───────────────┐
 │      YES      │──────→ Reuse & Improve
 └───────────────┘
...
Make it reusable
     ↓
Add it to CloudForge
     ↓
Reuse it in future projects
```

The goal is not to build every possible tool in advance.

The goal is to **capture useful solutions as they naturally appear in real projects**.
...
> The complete directory structure will be introduced gradually as functionality is added. The repository should not contain empty folders simply for the sake of having a predefined structure.
...
Yes. **`aws-workbench`** is a stronger name for this concept because it sounds like an actual engineering workspace rather than a fixed library. It also fits the idea that you will continuously experiment, build, test, refine, and promote reusable solutions.

I would structure it so the **README itself becomes the primary project reference for Kiro**: what the repository is, what belongs where, how utilities should be designed, how project-specific code is handled, AWS account/region conventions, security rules, naming, testing, documentation, and the long-term roadmap.

## Recommended repository structure

I would start with this structure rather than creating dozens of empty directories:
...
Then, **only when a real utility is created**, expand something like:
...
That keeps the repository clean from day one.
...
That will make Kiro much more useful because those files can contain **specific rules derived from how the workbench actually evolved**, rather than theoretical rules created before you have any utilities.
